...
Page({
  ...
  onLoad: function(options) {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        console.log('订单内商品列表:'+ordercommodity);
  ...
  queren:function(e){
    ...
    var orderid = e.currentTarget.dataset.orderid
    wx.request({
      url: 'http://127.0.0.1:8080/schoolsell/confirmOrder?orderID='+orderid,
      method:"GET",
      success:function(result){
        var string_result=JSON.stringify(result);
        var json_result=JSON.parse(string_result);
        console.log('确认付款返回值:'+json_result);
        if(json_result.result=='成功'){
            console.log("确认付款成功");
