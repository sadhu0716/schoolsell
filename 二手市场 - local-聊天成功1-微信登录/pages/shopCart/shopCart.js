...
Page({
  ...
  onShow: function() {
    ...
    wx.request({
      ...
      success: function (e) {
        var tecarts=[];
        var e2=JSON.stringify(e.data.result)
        var resu_temp=JSON.parse(e2);
        console.log("e.date的值为" + e2);
        console.log("resu_temp的长度为:"+resu_temp.length);
        console.log("购物车第一条信息标题:"+resu_temp[0].title);
        for(var i=0;i<resu_temp.length;i++){
          // var cartTitle='carts['+i+'].title';
          // var cartImage = 'carts[' + i + '].image';
          // var cartNum = 'carts[' + i + '].num';
          // var cartPrice = 'carts[' + i + '].price';
          // var cartCid = 'carts[' + i + '].cid';
          var tempcarts={
            'title': (resu_temp)[i].title,
            'image': (resu_temp)[i].image,
            'num': (resu_temp)[i].num,
            'price': (resu_temp)[i].price,
            'cid': (resu_temp)[i].cid
          }          
          tecarts.push(tempcarts)
        }
        console.log("购物车中列表信息:" + JSON.stringify(tecarts));
        self.setData({
          carts: tecarts
        })
