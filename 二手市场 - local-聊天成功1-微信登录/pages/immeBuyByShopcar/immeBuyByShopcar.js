...
Page({
  ...
  buybtn: function() {
    ...
    //   }
    // });
    console.log("购物车测试--商品集合:"+self.data.commditys);

    // 给服务器提交订单信息，生成待付款订单
    wx.request({
      url: 'http://127.0.0.1:8080/schoolsell/orderSuccessByShopCart',
      method: 'GET',
      data: {
        "orderDate": utils.datetimeFormat_1(new Date()),
        "buyerID": app.appData.userinfo.userID,        
        "buyerPhone": self.data.userphone,        
        "carts": JSON.stringify(self.data.commditys)       
      },
      success: function(res) {
        console.log("购物车下订单返回值: "+res);
        if(res.data.result=='成功'){
          var orderID=res.data.orderID;
          console.log("从购物车下订单返回的订单值: "+orderID);
          wx.navigateTo({
